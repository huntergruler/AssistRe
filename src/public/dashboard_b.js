            ...
            if (data.length === 0) {
                ...
                data.forEach(request => {
                    ...
                    if (request.buyerRequested == 1 && request.agentReply == '') {
                        ...
                        <button type="button" id="sendbuyerinfo-button" class="buyerinfo-button" onclick="sendBuyerInfo(${request.buyerid},${request.agentid});">
...
function offerDetail(agentid, buyeragentmatchid) {
    ...
    fetch(`/getOffers?agentid=${encodeURIComponent(agentid)}&buyeragentmatchid=${encodeURIComponent(buyeragentmatchid)}&datatype=${encodeURIComponent(datatype)}`)
        ...
        .then(data => {
            data.forEach(request => {
                const div = document.createElement("div");
                div.className = "form-row agentcontact-container";
                div.id = "agentid" + request.agentid;
                var innerHTMLtext = `<div class="flex-fill offerdetail">
    ...
    fetch(`/getagentinfo?agentid=${encodeURIComponent(agentid)}`)
        ...
        .then(data => {
            data.results.forEach(item => {
                ...
                <div  class="row col-md-10" style="border:1px solid black; border-radius:5px; margin-bottom: 2px"> ${item.bio}<br></div>
                </div>
