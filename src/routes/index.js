...
router.get('/session-data', (req, res) => {
  res.json({ user: req.session.user, firstname: req.session.firstname, userid: req.session.userid, lastname: req.session.lastname, userType: req.session.userType, buyerid: req.session.buyerid, agentid: req.session.agentid, paymentSuccessful: req.session.paymentSuccessful, userState: req.session.userState });
});
...
router.post('/setPaymentStatus', (req, res) => {
  if (!req.session.user) {
    req.session.message = 'Please login to save changes';
    res.redirect('/');
  }
...
router.get('/profile_b', (req, res) => {
  if (!req.session.user) {
    ...
    const userid = req.session.userid;
    const query = `SELECT b.userid, b.firstName, b.lastName, b.address, b.city, b.state, concat(b.city,', ',b.state) cityState, b.zip, b.email, b.phoneNumber, 
